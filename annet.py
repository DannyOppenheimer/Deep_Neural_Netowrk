...
class Brain:
    ...
    def __init__(self, layers, verbose=0, learning_rate=0.1):
        ...
        for i in range(len(neurons)):
            for neuron in neurons[i]:
                ...
                if i != len(neurons) - 1:
                    for forward_neuron in neurons[i + 1]:
    ...
    def back_propagate(self, l, outputs):
        ...
        for i in range(len(neurons[-1])):
            neurons[-1][i].error = outputs[l][i] - neurons[-1][i].value
            if self.verbose >= 2: print('Error of the output neuron', q, 'during output', l, 'is', neurons[-1][q].error)

        # Here we calculate the error of all the hidden layers' neurons
        for i, neuron_list in reversed(list(enumerate(neurons))):
            for neuron in neuron_list:
                if i != len(neurons) - 1 and i != 0:
                    ...
                    if self.verbose >= 2: print('Error of the hidden neuron in layer ', neurons.layer, 'during output', l, 'is', neuron.error)
    ...
    def input_output_checks(self, check_outputs, check_inputs):
        ...
        for input_iter in check_inputs:
            if len(input_iter) > len(neurons[0]) or len(input_iter) < len(neurons[0]):
        ...
        for output_iter in enumerate(check_outputs):
            # if len(output_iter) > len(neurons[-1]) or len(output_iter) < len(neurons[-1]):
            # self.shut_down_model("Number of outputs does not match the number of output neurons")

            # TODO: Fix below, non-functional
            for i, output_num in enumerate(input_iter):
